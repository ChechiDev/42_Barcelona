...
El flujo principal (`run_demo`) demuestra el escenario del enunciado: primero se registra solo el procesador numérico, luego se añaden los procesadores de texto y logs, se reprocesa el mismo stream, se consumen algunos elementos con `output()` mediante `put_processor_outputs()` y se muestran las estadísticas actualizadas.
...
- Incluye un escenario ejecutable con `python3 data_stream.py`.
...
En este entorno se confirmó `py_compile` y la ejecución del script. Según el recheck disponible, `pytest`, `flake8` y `mypy --strict` no están instalados, así que quedan como comprobación recomendada en un entorno completo.
