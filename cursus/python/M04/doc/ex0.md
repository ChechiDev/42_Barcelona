...
Mission Briefing: The Archives have discovered an ancient data fragment in Storage Vault 7. Your first assignment as a Data Archivist is to recover this precious information before it degrades further.

Get the name of a file from the command line, then read the file's contents and display them as the cat command would. Add a few headers and footers as shown in the example. You need to handle the various failure cases (nonexistent files, inaccessible files, etc.).
